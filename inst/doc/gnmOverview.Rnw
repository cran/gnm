...
The \Rfunarg{lsMethod} argument specifies what numerical
     method is to be used to solve the (typically rank-deficient) least
     squares problem at the heart of the \Rfunction{gnm} fitting algorithm: the
     options are  direct solution using a QR decomposition (\Rcode{``qr''}),
     and matrix inversion via Cholesky decomposition (\Rcode{``chol''}).  In
     both cases, the design matrix is standardized and regularized (in
     the Levenberg-Marquardt sense) prior to solving; the \Rfunarg{ridge}
     argument provides a degree of control over the regularization
     performed (smaller values may sometimes give faster convergence
     but can lead to numerical instability).  If \Rfunarg{lsMethod} is left
     unspecified, the default is \Rcode{``qr''}, unless \Rfunarg{eliminate}
     is used in
     which case the default \Rfunarg{lsMethod} used is \Rcode{``chol''}.
...
starting values for the predictors. These values are then used as the
response variable in a nonlinear least squares model with the right-hand side as
originally specified in the \Rfunarg{formula} argument to \Rfunction{gnm}. One
iteration of the \Rfunction{gnm} fitting algorithm is performed to estimate the
parameters in this model which are then used as starting values for the desired
\Rfunction{gnm} model.
...
\end{Soutput}
See Section \ref{sec:getContrasts} for further details of the
\Rfunction{getContrasts} function.
    ...
    \begin{fbox}
        {
          \begin{tabular*}{7.5cm}{@{\extracolsep{\fill}}lll@{\extracolsep{\fill}}}
              anova 	&	 formula 	&	print \\
              case.names	&	 hatvalues 	&	 profile \\
              coef	&	 labels 	&	 residuals \\
              cooks.distance	&	 logLik 	&	 rstandard \\
              confint	&	 model.frame 	&	 summary \\
              deviance	&	 model.matrix 	&	 variable.names \\
              extractAIC	&	 plot 	&	 vcov \\
              family	&	 predict	&	 weights \\
...
@ %def
which broadly agree with Table 8 of Chadouef and Denis (1991), allowing for the
change in sign.
...
@ %def
In this instance, only 37 of the 100 calls to \Rfunction{gnm} have successfully
located a local maximum of the likelihood: in the remaining 63 cases the
...
the last of these, the exact fit to the data, having been found 12 times
out of the above 37).
